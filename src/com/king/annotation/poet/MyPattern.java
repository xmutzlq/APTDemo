...
loop:   for(int x=0, offset=0; x<nCodePoints; x++, offset+=len) {
    ...
    static class LastNode extends Node {
         ...
         */
        boolean match(MyMatcher matcher, int i, CharSequence seq) {
            if (matcher.acceptMode == MyMatcher.ENDANCHOR && i != matcher.to)
                return false;
            matcher.last = i;
            matcher.groups[0] = matcher.first;
            matcher.groups[1] = matcher.last;
    ...
    static final class Curly extends Node {
        ...
        boolean match0(MyMatcher matcher, int i, int j, CharSequence seq) {
            if (j >= cmax) {
        ...
        // i is the index to start matching at
        // j is the number of atoms that have matched
        boolean match1(MyMatcher matcher, int i, int j, CharSequence seq) {
            for (;;) {
        ...
        boolean match2(MyMatcher matcher, int i, int j, CharSequence seq) {
            for (; j < cmax; j++) {
                if (!atom.match(matcher, i, seq))
                    break;
                if (i == matcher.last)
    ...
    static final class GroupHead extends Node {
        ...
        boolean matchRef(MyMatcher matcher, int i, CharSequence seq) {
            int save = matcher.locals[localIndex];
    ...
    static class Loop extends Node {
        ...
        boolean matchInit(MyMatcher matcher, int i, CharSequence seq) {
    ...
    static final class Bound extends Node {
        ...
        int check(MyMatcher matcher, int i, CharSequence seq) {
            int ch;
            boolean left = false;
            int startIndex = matcher.from;
            int endIndex = matcher.to;
     ...
     */
    private static boolean hasBaseCharacter(MyMatcher matcher, int i,
                                            CharSequence seq)
    {
...
//    }
    ...
    public Stream<String> splitAsStream(final CharSequence input) {
        class MatcherIterator implements Iterator<String> {
            private final MyMatcher matcher;
