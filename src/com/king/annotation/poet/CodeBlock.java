 ...
 * quotes, and emits that. For example, {@code 6" sandwich} is emitted {@code "
 * 6\" sandwich"}.
...
public final class CodeBlock {
	private static final MyPattern NAMED_ARGUMENT = MyPattern.compile("\\$(?<argumentName>[\\w_]+):(?<typeChar>[\\w]).*");
	private static final MyPattern LOWERCASE = MyPattern.compile("[a-z]+[\\w_]*");
	...
	public static final class Builder {
		 ...
		 *
		 * <p>
		 * Named arguments specify their name after the '$' followed by : and
		 * the corresponding type character. Argument names consist of
		 * characters in {@code a-z, A-Z, 0-9, and _} and must start with a
		 * lowercase character.
		 *
		 * <p>
		 * For example, to refer to the type {@link java.lang.Integer} with the
		 * argument name {@code
		 * clazz} use a format string containing {@code $clazz:T} and include
		 * the key {@code clazz} with value {@code java.lang.Integer.class} in
		 * the argument map.
		 */
		public Builder addNamed(String format, Map<String, ?> arguments) {
			...
			while (p < format.length()) {
				...
				if (matcher != null && matcher.lookingAt()) {
					...
					checkArgument(isNoArgPlaceholder(format.charAt(p + 1)), "unknown format $%s at %s in '%s'",
		 ...
		 *
		 * <p>
		 * Relative arguments map 1:1 with the placeholders in the format
		 * string.
		 *
		 * <p>
		 * Positional arguments use an index after the placeholder to identify
		 * which argument index to use. For example, for a literal to reference
		 * the 3rd argument: "$3L" (1 based index)
		 *
		 * <p>
		 * Mixing relative and positional arguments in a call to add is invalid
		...
		public Builder add(String format, Object... args) {
			...
			for (int p = 0; p < format.length();) {
				...
				if (indexStart < indexEnd) {
					...
					hasIndexed = true;
					if (args.length > 0) {
						indexedParameterCount[index % args.length]++; // modulo
																		// is
																		// needed,
																		// checked
																		// below
