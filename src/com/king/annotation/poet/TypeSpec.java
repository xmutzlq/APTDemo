...
import static com.king.annotation.poet.Util.checkNotNull;
import static com.king.annotation.poet.Util.checkArgument;
import static com.king.annotation.poet.Util.checkState;
import static com.king.annotation.poet.Util.hasDefaultModifier;
import static com.king.annotation.poet.Util.requireExactlyOneOf;
...
public final class TypeSpec {
		...
		try {
			if (enumName != null) {
				...
				if (kind == Kind.INTERFACE) {
					...
					extendsTypes = superclass.equals(ClassName.OBJECT) ? Collections.<TypeName> emptyList()
			...
			for (Iterator<Map.Entry<String, TypeSpec>> i = enumConstants.entrySet().iterator(); i.hasNext();) {
				...
				if (!firstMember)
					codeWriter.emit("\n");
				enumConstant.getValue().emit(codeWriter, enumConstant.getKey(), Collections.<Modifier> emptySet());
				firstMember = false;
				if (i.hasNext()) {
					codeWriter.emit(",\n");
	...
	public enum Kind {
		CLASS(Collections.<Modifier> emptySet(), Collections.<Modifier> emptySet(), Collections.<Modifier> emptySet(),
				Collections.<Modifier> emptySet()),

		INTERFACE(Util.immutableSet(Arrays.asList(Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)),
		...
		ENUM(Collections.<Modifier> emptySet(), Collections.<Modifier> emptySet(), Collections.<Modifier> emptySet(),
				Collections.singleton(Modifier.STATIC)),

		ANNOTATION(Util.immutableSet(Arrays.asList(Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)),
				Util.immutableSet(Arrays.asList(Modifier.PUBLIC, Modifier.ABSTRACT)),
				Util.immutableSet(Arrays.asList(Modifier.PUBLIC, Modifier.STATIC)),
				Util.immutableSet(Arrays.asList(Modifier.STATIC)));
		...
		Kind(Set<Modifier> implicitFieldModifiers, Set<Modifier> implicitMethodModifiers,
	...
	public static final class Builder {
		...
		public Builder addField(FieldSpec fieldSpec) {
			if (kind == Kind.INTERFACE || kind == Kind.ANNOTATION) {
				...
				checkState(fieldSpec.modifiers.containsAll(check), "%s %s.%s requires modifiers %s", kind, name,
						fieldSpec.name, check);
			}
		...
		public Builder addMethod(MethodSpec methodSpec) {
			if (kind == Kind.INTERFACE) {
				...
				checkState(methodSpec.modifiers.equals(kind.implicitMethodModifiers), "%s %s.%s requires modifiers %s",
						kind, name, methodSpec.name, kind.implicitMethodModifiers);
			}
			if (kind != Kind.ANNOTATION) {
				checkState(methodSpec.defaultValue == null, "%s %s.%s cannot have a default value", kind, name,
						methodSpec.name);
			}
			if (kind != Kind.INTERFACE) {
				checkState(!hasDefaultModifier(methodSpec.modifiers), "%s %s.%s cannot be default", kind, name,
						methodSpec.name);
			}
		...
		public Builder addType(TypeSpec typeSpec) {
			checkArgument(typeSpec.modifiers.containsAll(kind.implicitTypeModifiers), "%s %s.%s requires modifiers %s",
